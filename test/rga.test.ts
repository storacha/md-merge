...
import { RGA, type RGANodeId, type RGAEvent, type EventComparator } from '../src/crdt/rga.js'
...
describe('RGA merge', () => {
  ...
  it('merge is commutative', () => {
    ...
    m1.nodes.set(`${b1Id.uuid}:${b1Id.event}`, rep1.nodes.get(`${b1Id.uuid}:${b1Id.event}`)!)
    m1.nodes.set(`${b2Id.uuid}:${b2Id.event}`, rep2.nodes.get(`${b2Id.uuid}:${b2Id.event}`)!)

    const m2 = makeReplica()
    m2.nodes.set(`${b2Id.uuid}:${b2Id.event}`, rep2.nodes.get(`${b2Id.uuid}:${b2Id.event}`)!)
    m2.nodes.set(`${b1Id.uuid}:${b1Id.event}`, rep1.nodes.get(`${b1Id.uuid}:${b1Id.event}`)!)
